...
│   └── mode/                # Mode-specific tokens (colors, typography, etc.)
...
1. **Source**: Token definitions in `data/tokens/` (JSON format)
2. **Build**: `npm run build` processes tokens through Style Dictionary
3. **Output**: Generated files in `dist/` across multiple formats:
   - CSS custom properties (`dist/css/`)
   - SCSS variables (`dist/scss/`)
   - CommonJS modules (`dist/js/common/`)
   - ES6 modules (`dist/js/es6/`)
   - JSON (`dist/json/`)

### Token organization

Tokens are organized hierarchically by component and theme. For example:
...
npm test -- tests/components.test.ts
```
