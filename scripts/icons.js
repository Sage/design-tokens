...
const { join, relative, resolve, sep, posix } = require("path");
...
async function getIconsArray(config) {
  console.log(
    `Fetching icon components information from Figma file ${config.fileId}...`
  );
  ...
  const components = await client.file(config.fileId).then(({ data }) => {
    ...
    let canvases = data.document.children.filter(
    ...
    const groupedComponents = Object.fromEntries(
      canvases.map((canvas) => [
        canvas.name,
        collect(canvas, verifyFn).map((node) => node.id),
      ])
    );
...
async function createWebFonts(iconsList, config) {
  ...
  const sets = config.multipleSets
    ? uniqueValues(iconsList, (item) => item.set)
    : [""];
  return await Promise.all(
    sets.map(async (set) => {
      const icons = config.multipleSets
        ? iconsList.filter((icon) => icon.set === set)
        : iconsList;
      const name = config.multipleSets
        ? `${config.fontName}-${set}`
        : config.fontName;
      const inputDir = config.multipleSets
        ? resolve(".", config.svgDir, kebabCase(set))
        : resolve(".", config.svgDir);
      const outputDir = resolve(".", config.fontsDir);
      const codepoints = Object.fromEntries(
        icons
          .filter((icon) => !!icon.unicode)
          .map((icon) => [kebabCase(icon.name), parseInt(icon.unicode, 16)])
      );
      ...
      return generateFonts({
        ...
        console.log(
          `  - ${name} set formats written in ${outputDir} in ${config.formats.length} formats`
        );
        ...
        return icons.map((icon) => {
          ...
          return {
            ...icon,
            path: relative(config.distDir, icon.path)
              .split(sep)
              .join(posix.sep),
            codepoint,
...
async function writeGlyphsData(glyphsData, config) {
  ...
  if (config.multipleSets) {
    ...
    console.log(
      `  Writing data for ${glyphsData.length} icons in ${sets.length} sets`
    );
  } else {
...
(async (config) => {
  if (!config.fileId || !config.personalAccessToken) {
    console.error(
      "Icons will not be generated, since token and figma file id were not found."
    );
    console.error(
      "Please provide FIGMA_ACCESS_TOKEN and FIGMA_FILE_ID env variables or in .env file.\r\n"
    );
    return;
  }
