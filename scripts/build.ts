...
const getFormat = (format: string, outputRefs: IFiles["outputRefs"], componentName: string): string => {
  const hasOutputRefs = typeof outputRefs === "function" || outputRefs === true

  // outputRefs is true for mode and component files, false for global
  if (format === "json/flat" && hasOutputRefs) {
