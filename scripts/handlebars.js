...
Handlebars.registerHelper(
  "prefixedPartial",
  function (context, prop, prefix, options) {
    const partialName = kebabCase(
      prefix ? `${prefix}-${context[prop]}` : context[prop]
    );
    const partial = Handlebars.partials[partialName];

    if (!partial) {
      return `Partial ${partialName} is missing. (${Object.keys(
        Handlebars.partials
      )})`;
    }
...
module.exports = (pattern) => {
  ...
  glob(pattern).forEach((path) => {
    const name = path.replace(prefix, "").replace(suffix, "").replace("/", "-");
